...
async function getRecsFromAPI(data, userId, filter, operator) {
  const response = await fetch(
    ` https://api.themoviedb.org/3/discover/tv?api_key=${process.env.TMDB_API_KEY}&language=en-US&sort_by=popularity.desc&page=1&timezone=America%2FNew_York&with_${filter}=${data[0]}${operator}${data[1]}${operator}${data[2]}&include_null_first_air_dates=false`
  );
...
async function getRecsByGenres(userId) {
  ...
  const filter = 'genres';
  const operator = '%2C';
  const recsByGenres = await getRecsFromAPI(genres, userId, filter, operator);
  return recsByGenres;
}
...
async function getRecsByNetworks(userId) {
  ...
  const operator = '%7C';
  const recsByNetworks = await getRecsFromAPI(
    networks,
    userId,
    filter,
    operator
