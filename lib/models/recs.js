...
async function getRecsFromAPI(data, userId, filter, operator, pageNumber) {
  const response = await fetch(
    ` https://api.themoviedb.org/3/discover/tv?api_key=${process.env.TMDB_API_KEY}&language=en-US&sort_by=popularity.desc&page=${pageNumber}&timezone=America%2FNew_York&with_${filter}=${data[0]}${operator}${data[1]}${operator}${data[2]}&include_null_first_air_dates=false`
  );
  const unfilteredRecs = await response.json();
  const paginationInformation = unfilteredRecs.total_pages.toString();
  ...
  return [filteredRecsListView, paginationInformation];
}

async function getRecsByGenres(userId, pageNumber) {
  const genres = await getMostWatchedGenres(userId);
  const filter = 'genres';
  const operator = '%2C';
  const [recs, paginationInformation] = await getRecsFromAPI(
    genres,
    userId,
    filter,
    operator,
  ...
  return [recs, paginationInformation];
}

async function getRecsByNetworks(userId, pageNumber) {
