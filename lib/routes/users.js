...
const { getRecsByGenres, getRecsByNetworks } = require('../models/recs');
...
router.get('/:id/recs/genres/:pageNumber', async (request, response) => {
  ...
  try {
    const [recs] = await getRecsByGenres(userId, pageNumber);
    response.send(recs);
  } catch (error) {
    throw new Error(error.message);
  }
...
router.get('/:id/recs/genres/', async (request, response) => {
  ...
  const pageNumber = 1;
  try {
    const [, paginationInformation] = await getRecsByGenres(userId, pageNumber);
    response.send(paginationInformation);
  } catch (error) {
    throw new Error(error.message);
  }
});
...
router.get('/:id/recs/networks/:pageNumber', async (request, response) => {
  ...
  const pageNumber = request.params.pageNumber;
  try {
    const [recs] = await getRecsByNetworks(userId, pageNumber);
    response.send(recs);
  } catch (error) {
    throw new Error(error.message);
  }
...
router.get('/:id/recs/networks/', async (request, response) => {
  const userId = request.params.id;
  const pageNumber = 1;
  try {
    const [, paginationInformation] = await getRecsByNetworks(
      userId,
      pageNumber
    );
    response.send(paginationInformation);
